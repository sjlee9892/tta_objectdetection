...
class getFrequency:
    ...
    def getClassFreq(self, is_valid=False):
        ...
        if is_valid:
            save_path = os.path.join(self.save_dir, 'TEST_DATA_FREQUENCY.csv')
            label_txt_list = self.getValidTxt()
        else:
            save_path = os.path.join(self.save_dir, 'TOTAL_DATA_FREQUENCY.csv')
            label_txt_list = os.listdir(self.LABELS_FOLDER_PATH)

        for txt_name in tqdm(label_txt_list):
